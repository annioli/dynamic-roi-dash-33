...
export const VariableDebtsCard: React.FC = () => {
    ...
    <Card className="bg-gradient-to-br from-slate-900/50 to-slate-800/30 backdrop-blur-xl border border-slate-700/50 shadow-2xl">
      <CardHeader>
        <CardTitle className="flex items-center space-x-3 text-white">
          ...
          <span>Outras Dívidas</span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        {data.variableDebts.length === 0 ? (
          <div className="text-center py-8">
            <CreditCard className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <p className="text-slate-400">Nenhuma dívida cadastrada</p>
          </div>
        ) : (
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {data.variableDebts.map((debt) => (
              <div
                key={debt.id}
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-400">Vencimento: {formatDate(debt.dueDate)}</p>
                    {debt.description && (
                      <p className="text-xs text-slate-500 mt-1">{debt.description}</p>
                    )}
                  </div>
                  <div className="text-right">
                    <div className={`font-bold ${debt.isPaid ? 'text-green-400' : 'text-orange-400'}`}>
                ...
                <div className="flex space-x-2">
                  <Button
                    size="sm"
                    onClick={() => toggleDebtPayment(debt.id, 'variable')}
                    disabled={loading}
                    className={`flex-1 ${
                      debt.isPaid
                  ...
                  <Button
                    size="sm"
                    onClick={() => removeDebt(debt.id, 'variable')}
                    disabled={loading}
